...
function generateSphere(vertices, edges, radius) {
    ...
    scene.add(new THREE.AxisHelper(radius*1.5));
...
function projectOntoMesh(point, mesh) {
    ...
    var direction = point.clone().negate().normalize();
...
function init() {
    ...
    scene.add(camera);
    camera.position.set(0,150,4*radius);
    ...
    var skyBoxGeometry = new THREE.CubeGeometry(FAR*0.5,FAR*0.5,FAR*0.5);
